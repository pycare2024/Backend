...
// ✅ 1. Upload Excel and Parse it
EmailRoute.post("/send-bulk", upload.fields([
    ...
    console.log("📎 Attachment File:", attachmentFile?.originalname);

    if (!excelFile || !subject || !template) {
        return res.status(400).json({ message: "Missing required fields (Excel, subject, or template)" });
    }
    ...
    try {
        ...
        if (!recipients.length) {
            return res.status(400).json({ message: "No recipients found in Excel." });
        }

        const attachments = attachmentFile
            ? [{
                filename: attachmentFile.originalname,
                path: path.resolve(attachmentFile.path)
            }]
            : [];
        ...
        for (const recipient of recipients) {
            ...
            for (const placeholder of placeholders) {
                ...
                personalizedHtml = personalizedHtml.replaceAll(placeholder, recipient[key] || "");
            }

            try {
                const response = await sendEmail(email, subject, personalizedHtml, attachments);
        ...
        res.status(200).json({
            message: results.length > 0 ? "Emails processed." : "No emails were sent. Please check your Excel file and placeholders.",
            results
        });
