...
const fetch = require("node-fetch"); // Using fetch instead of axios
...
OtpRoute.get("/send-otp/:phoneNumber", async (req, res) => {
    const { phoneNumber } = req.params; // Get phone number from URL
    ...
    try {
        const response = await fetch(WATI_API_URL, {
            ...
            body: JSON.stringify({
                number: `91${phoneNumber}`, // Prefix '91' for India
